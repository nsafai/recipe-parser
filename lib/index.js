...
function parse(recipeString) {
    ...
    if (quantity && quantity.includes('-')) {
        _a = quantity.split('-').map(function (it) { return it.trim(); }), minQty = _a[0], maxQty = _a[1];
    ...
    * if there is a number at the beginning we do not have a unit because the unit is assumed to be at the beginning*/
    var startsWithNumberRegex = /^\d+(\.\d+|\s+\d+\/\d+)?(\s+\w+\.?|\w+\.?\s+)/;
    var extraUnitInfo = '';
    var numberWithNextWord = convert.getFirstMatch(ingredient, startsWithNumberRegex).trim();
    if (numberWithNextWord.split(' ').length > 1) {
        var tempUnit = getUnit(numberWithNextWord.split(' ')[1])[0];
        if (tempUnit) {
            unit = tempUnit;
        ...
        var numberOnlyRegex = /^\d+((\.\d+)|(\s+\d+\/\d+))?/;
        var num = convert.getFirstMatch(numberWithNextWord, numberOnlyRegex);
        var tempUnit = getUnit(numberWithNextWord.replace(num, ''))[0];
        if (tempUnit) {
            unit = tempUnit;
            extraUnitInfo = num;
            ingredient = ingredient.replace(numberWithNextWord, '').trim();
