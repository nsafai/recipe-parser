...
function findQuantityAndConvertIfUnicode(ingredientLine) {
    ...
    var numericRangeWithSpaceRegex = /^\d+((\.\d+)|(\s+\d+\/\d+))?\s*?(-|(to))\s*?\d+((\.\d+)|(\s+\d+\/\d+))?/g; // for ex: "1 to 2" or "1 - 2"
    ...
    if (ingredientLine.match(numericRangeWithSpaceRegex)) {
        var quantity = getFirstMatch(ingredientLine, numericRangeWithSpaceRegex).replace('to', '-').split(/\s*?-\s*?/).join('-');
